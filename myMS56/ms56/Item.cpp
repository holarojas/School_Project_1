...
namespace sdds
{
	...
	//Get the values for the Item object from the file
	std::ifstream& Item::load(std::ifstream& ifstr)
	{
		...
		if (ifstr.peek() != EOF) {
		//The tab separated SKU, description, on-hand quantity, needed quantity and price are read into 
		//their corresponding attributes and then single character is discarded from the file.
			string temp = { 0 };
			...
			int beginPos = ifstr.tellg(); //Get the initial position
			getline(ifstr, temp, '\t');
	...
	//Will first prompt  the user with the title "AMA Item" and then displays the SKU and reads the rest of 
	// the information in a full-proof way (the same as SKU)
