...
namespace sdds
{
	...
	{
		if (m_fileName != nullptr) {
			ofstream file(m_fileName, ios::app); //Appending at the end
	...
	bool AidMan::load()
	{
		...
		//Saves all the  already existing products
		//save();	
	...
	int AidMan::list(const char* sub_desc)
	{
		...
		if (!isEmpty) {
			if (sub_desc == nullptr) { //If nullptr, print all the items in linear format
				...
				//Print only the items containing the sub_desc in their desc
